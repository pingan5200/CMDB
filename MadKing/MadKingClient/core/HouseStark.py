...
class ArgvHandler(object):
    ...
    def __submit_data(self, action_type, data, method):
        ...
        if action_type in settings.Params['urls']:
            # /asset/report/asset_with_no_asset_id/ or /asset/report/
            if type(settings.Params['port']) is int:
                # 192.168.1.0:8000/asset/report/
                url = "http://%s:%s%s" %(settings.Params['server'], settings.Params['port'], settings.Params['urls'][action_type])
            else:
                # 192.168.1.0/asset/report/
                url = "http://%s%s" %(settings.Params['server'], settings.Params['urls'][action_type])
            # 修改url，返回带有ip地址 + 端口 + /asset/report/ + api安全验证的url，如：192.168.1.0:8000/asset/report/?user=lcs&timestamp=021546&token=eqwgch
            ...
            if method == "get":
                ...
                # 收集的所有数据 k1=v1&k2=v3&....
                args = args[1:]
                # 拼接 安全验证+数据 的url 如：192.168.1.0:8000/asset/report/?user=lcs&timestamp=021546&token=eqwgch&k1=v1&k2=v3
                url_with_args = "%s?%s" %(url, args)
                try:
                    # get 服务器 api
                    req = request.urlopen(url_with_args, timeout=settings.Params['request_timeout'])
                    callback = req.read()
                ...
                try:
                    ...
                    # url: 192.168.1.0:8000/asset/report/?user=lcs&timestamp=021546&token=eqwgch
                    # post 服务器 api
                    res_data = request.urlopen(url=url, data=data.encode('utf-8'), timeout=settings.Params['request_timeout'])
                    callback = res_data.read().decode('utf-8')
    ...
    def log_record(self, log, action_type=None):
        f = open(settings.Params["log_file"],"w+")
        if log is str:
            pass
        if type(log) is dict:
